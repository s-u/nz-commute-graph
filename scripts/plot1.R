...
d=readRDS("artifacts/sa2.rds") ## from data/statistical-area-2-*-clipped-generalised.shp
geo=st_geometry(d)
...
i = 1
    ...
    lsl  = st_as_sf(data.frame(x=a[[i]][,2], y=a[[i]][,1]), coords=c("x","y"), crs=4326)
...
if (for.paper) pdf("visualizations/plot1.pdf", 7, 7)

par(mar=rep(0,4))
plot(geoll[trs])
osmap(cache.dir="tmp/osm")
plot(geoll[trs], add=T, lwd=1, col=paste0(brewer.pal(9, "Set1"), "30"))
segments(a[[i]][-length(fwid),2], a[[i]][-length(fwid),1],
         a[[i]][-1,2], a[[i]][-1,1],
